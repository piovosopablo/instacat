...
// Este componente genera las opciones para votar por la imagen del gato del día y opción de gato favorito.
const CardVote = ({ onClickVote, onClickFavorite, catsOfTheDay, error }) => {
  {
    if (catsOfTheDay.length > 0) {
      return catsOfTheDay.map((cat) => (
        <div className='card_vote' key={cat.id}>
          <figure>
            <img
              className={
                cat.width > cat.height ?
                  'cat_image cat_image_horizontal' :
                  'cat_image cat_image_vertical'
              }
              src={cat.url}
              alt='Cat'
              srcSet=''
            ...
            <div className='card_vote_actions'>
              ...
              <button
                ...
                type='button'
                className='btn-card'
                onClick={() => onClickFavorite(cat.id)}
              >
    ...
    return <CardLoading error={error} source='cat of the day' />;
  }
};
